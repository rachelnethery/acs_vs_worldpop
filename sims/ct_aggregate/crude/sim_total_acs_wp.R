...
x_2010<-read.csv(paste0('merged',2010,'_fullpop.csv'),stringsAsFactors = F)

## aggregate across age/sex groups ##
x_2010 <- aggregate(acs_pop~GEOID+race,data=x_2010,
                     FUN = sum,na.rm=T)

## extract number black per CT ##
n_black<-dcast(subset(x_2010,race=='black' | race=='total',select=c('GEOID','race','acs_pop')),GEOID~race)
...
## subset to only total population (not race-stratified) ##
adat<-subset(adat,race=='total')
