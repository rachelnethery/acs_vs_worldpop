...
adat<-read.csv(paste0('merged',yr,'_fullpop.csv'),stringsAsFactors = F)

## subset to only black and white ##
adat<-subset(adat,race %in% c('white','black'))

## aggregate across age/sex groups ##
agg_pop <- aggregate(cbind(acs_pop,ce_pop,wp_pop)~GEOID+race,data=adat,
                     FUN = sum,na.rm=T)

acs_cov <- aggregate(acs_pov_pct~GEOID+race,data=adat,
                     FUN = mean)

## merge these aggregated datasets ##
adat<-merge(agg_pop,acs_cov,by=c('GEOID','race'))

## remove CTs with 0 pop ##
adat<-subset(adat,acs_pop>0 & ce_pop>0 & wp_pop>0)
...
P1<-matrix(adat$acs_pop)
P2<-matrix(adat$wp_pop)
...
car1<-matrix(NA,nrow=reps,ncol=length(beta)*3+9)
car2<-matrix(NA,nrow=reps,ncol=length(beta)*3+9)
...
for (xx in 1:reps){
  ...
  ####################################
  adat$Y<-Y
  adat$offs1<-log(lambda*P1)
  adat$offs2<-log(lambda*P2)
  car_offs1<-S.CARmultilevel(Y~bw_ind+acs_pov_pct+offset(offs1),family="poisson",data=adat,
                                ind.area=as.numeric(as.factor(adat$GEOID)),
                                ind.re=as.factor(1:N),
                                W=W,burnin=nburn,n.sample=nsamp)
  
  car_offs2<-S.CARmultilevel(Y~bw_ind+acs_pov_pct+offset(offs2),family="poisson",data=adat,
                             ind.area=as.numeric(as.factor(adat$GEOID)),
                             ind.re=as.factor(1:N),
                             W=W,burnin=nburn,n.sample=nsamp)
  ...
  car1[xx,]<-c(apply(car_offs1$samples$beta,2,mean),
               ...
               apply(car_offs1$samples$rho,2,mean),
               apply(car_offs1$samples$rho,2,quantile,.025),
               apply(car_offs1$samples$rho,2,quantile,.975))
  ...
  car2[xx,]<-c(apply(car_offs2$samples$beta,2,mean),
               ...
               apply(car_offs2$samples$rho,2,mean),
               apply(car_offs2$samples$rho,2,quantile,.025),
               apply(car_offs2$samples$rho,2,quantile,.975))
...
save(car1,car2,file=paste0('results/sim_rs_',yr,'_',simnum,'.RData'))
