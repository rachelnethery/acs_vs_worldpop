##############################################################
## extract ACS and census denominator data using tidycensus ##
...
for (yr in 2009:2012){
  ...
  ma_acs<- get_acs(geography = "tract",
                   variables = c(## total pop <65
                     paste0('B01001_',str_pad(as.character(c(3:25,27:49)),width=3,side='left',pad='0')),
                     ## non-hispanic white <65
                     paste0('B01001H_',str_pad(as.character(c(3:16,18:31)),width=3,side='left',pad='0')),
                     ## black <65
                     paste0('B01001B_',str_pad(as.character(c(3:16,18:31)),width=3,side='left',pad='0')),
                     ## american indian/alaska native <65
                     paste0('B01001C_',str_pad(as.character(c(3:16,18:31)),width=3,side='left',pad='0')),
                     ## asian <65
                     paste0('B01001D_',str_pad(as.character(c(3:16,18:31)),width=3,side='left',pad='0')),
                     ## native hawaiian/pacific islander
                     paste0('B01001E_',str_pad(as.character(c(3:16,18:31)),width=3,side='left',pad='0')),
                     ## hispanic
                     paste0('B01001I_',str_pad(as.character(c(3:16,18:31)),width=3,side='left',pad='0'))
                   ),
                   state = "MA",year=yr)
  ...
  ## age ranges ##
  agecat<-paste0('Age',c('0-4','5-9','10-14','15-19','20-24','25-29','30-34','35-44','45-54','55-64','65-74','75+'))
  
  rxsxa<-ma_acs[!duplicated(ma_acs[,c('racevar','vnum')]),c('racevar','vnum')]
  rxsxa<-rxsxa[order(rxsxa$racevar,rxsxa$vnum),]
  
  #realign_cenage<-c(1:5,6,6,6,7,8,9,9,10,10,11,11,11)
  realign_totage<-c(1,2,3,4,4,5,5,5,6,7,8,8,9,9,10,10,10,11,11,11,12,12,12)
  realign_rsage<-c(1,2,3,4,4,5,6,7,8,9,10,11,12,12)
  rxsxa$age<-c(rep(realign_totage,2),rep(realign_rsage,2*(length(unique(rxsxa$racevar))-1)))
  rxsxa$sex<-c(rep(c('m','f'),each=length(realign_totage)),rep(rep(c('m','f'),each=length(realign_rsage)),length(unique(rxsxa$racevar))-1))
...
save(acs_list,file='acs_data_fullpop.RData')
rm(list=ls())
...
nums65<-str_pad(c(3:25,27:49),width=3,side='left',pad='0')
...
realign_totage<-c(1,2,3,4,4,5,5,5,6,7,8,8,9,9,10,10,10,11,11,11,12,12,12)
rxsxa$age<-rep(realign_totage,2*(length(unique(rxsxa$racevar))))
rxsxa$sex<-rep(rep(c('m','f'),each=length(realign_totage)),length(unique(rxsxa$racevar)))
rxsxa<-merge(rxsxa,data.frame('age'=1:length(agecat),'agecat'=agecat),by='age')
...
save(ma_ce,file='ce_data_fullpop.RData')
